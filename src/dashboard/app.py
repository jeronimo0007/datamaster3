...
SECTION_KAFKA = "Kafka / eventos do dia"
...
section = st.radio(
    "Seção",
    [SECTION_TX, SECTION_BATCH, SECTION_KAFKA, SECTION_LGPD, SECTION_CHAT, SECTION_CHARTS],
    horizontal=True,
    key="main_section",
...
elif section == SECTION_KAFKA:
    st.subheader("Kafka — eventos `transaction-analyzed` do dia")
    st.markdown(
        """
Após cada `POST /analyze`, a API responde o score e **publica o evento no Kafka de forma assíncrona**.
O payload traz **CPF** e apenas o **final do cartão** (`card_last4`).
O mesmo evento fica espelhado em Mongo (`analyzed_events`) para consulta no painel e no Jupyter
(`notebooks/02_kafka_consultas_dia.ipynb`).
        """
    )
    day = st.text_input("Dia (today / YYYY-MM-DD)", value="today")
    data = fetch_data(f"/api/v1/events/analyzed?date={day}") or {}
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Dia", data.get("event_day", "—"))
    m2.metric("Total eventos", data.get("total", 0))
    m3.metric("Fraudes", data.get("frauds", 0))
    m4.metric("Legítimos", data.get("legit", 0))
    st.caption(f"Tópico Kafka: `{data.get('kafka_topic', 'transaction-analyzed')}`")

    rows = data.get("events") or []
    if not rows:
        st.info(
            "Sem eventos neste dia. Envie uma análise pela sidebar ou:\n\n"
        ...
        st.dataframe(df_ev[show_cols], use_container_width=True, hide_index=True)
        ...
        with c_a:
            fraud_map = (
                ...
                .fillna(False)
                .astype(bool)
                .map({True: "Fraude", False: "Legítimo"})
                .value_counts()
                .reset_index()
            )
            fraud_map.columns = ["classe", "qtd"]
            st.plotly_chart(
                px.pie(fraud_map, names="classe", values="qtd", title="Fraude × Legítimo"),
                use_container_width=True,
            )
        with c_b:
            if "fraud_score" in df_ev.columns:
                st.plotly_chart(
                    px.histogram(
                        df_ev,
                        x="fraud_score",
                        nbins=12,
                        title="Distribuição de fraud_score",
                    ),
                    use_container_width=True,
                )
